...
namespace Microsoft.Extensions.HealthChecks
{
    public abstract class CachedHealthCheck
    {
        ...
        public async ValueTask<IHealthCheckResult> RunAsync(IServiceProvider serviceProvider, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            {
                // Can't use a standard lock here because of async, so we'll use this flag to
                // determine when we should write a value, and the waiters who aren't allowed to
                // write will just spin wait for the new value.
